...
select name, some_date, id from {{ source('raw', 'seed') }}
{% if is_incremental() %}
where id > (select max(id) from {{ this }})
{% endif %}
""".strip()

class TestIncrementalPartitionImpala(TestIncrementalImpala):
    @pytest.fixture(scope="class")
    def models(self):
        return {
            "incremental_test_model.sql": incremental_partition_sql,
            "schema.yml": schema_base_yml
        }

incremental_multiple_partition_sql = """
{{
    config(
        materialized="incremental",
...
{% endif %}
""".strip()
class TestIncrementalMultiplePartitionImpala(TestIncrementalImpala):
    @pytest.fixture(scope="class")
    def models(self):
        return {
            "incremental_test_model.sql": incremental_multiple_partition_sql,
            "schema.yml": schema_base_yml
        }
