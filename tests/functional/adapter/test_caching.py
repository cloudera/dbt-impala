...
# Override the base caching logic to safely handle None values during lower() comparisons.
# In Hive/Impala, the 'database' field may be unset or unused in metadata, so we skip


class PatchedCachingTest(BaseCachingTest):
...
class TestCachingLowerCaseModel(PatchedCachingTest, BaseCachingLowercaseModel):
    pass


class TestCachingUppercaseModel(PatchedCachingTest, BaseCachingUppercaseModel):
    pass


class TestCachingSelectedSchemaOnly(PatchedCachingTest, BaseCachingSelectedSchemaOnly):
    pass


class TestHiveNoPopulateCache(PatchedCachingTest, BaseNoPopulateCache):
    pass
